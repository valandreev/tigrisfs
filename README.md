...
<img src="doc/tigrisfs.png" height="64" width="64" align="middle" />

TigrisFS is a high-performance, POSIX-ish S3
([Yandex](https://cloud.yandex.com/en-ru/services/storage), [Amazon](https://aws.amazon.com/s3/))
file system written in Go

# Overview

TigrisFS allows you to mount an S3 bucket as a file system.
...
|                   | TigrisFS | rclone | Goofys | S3FS | gcsfuse |
| ----------------- | ------- | ------ | ------ | ---- | ------- |
| Read after write  |    +    |    +   |    -   |   +  |    +    |
| Partial writes    |    +    |    +   |    -   |   +  |    +    |
| Truncate          |    +    |    -   |    -   |   +  |    +    |
...
List of non-POSIX behaviors/limitations for TigrisFS:
...
TigrisFS is stable enough to pass most of `xfstests` which are applicable,
...
* Pre-built binaries:
  * [Linux amd64](https://github.com/yandex-cloud/tigrisfs/releases/latest/download/tigrisfs-linux-amd64).
    You may also need to install FUSE utils (fuse3 or fuse RPM/Debian package) first.
  * [Mac amd64](https://github.com/yandex-cloud/tigrisfs/releases/latest/download/tigrisfs-mac-amd64),
    [arm64](https://github.com/yandex-cloud/tigrisfs/releases/latest/download/tigrisfs-mac-arm64). You also need osxfuse/macfuse for TigrisFS to work.
  * [Windows x64](https://github.com/yandex-cloud/tigrisfs/releases/latest/download/tigrisfs-win-x64.exe).
    You also need to install [WinFSP](https://winfsp.dev) first.
* Or build from source with Go 1.13 or later:

```ShellSession
$ git clone https://github.com/yandex-cloud/tigrisfs
$ cd tigrisfs
$ go build
```
...
$ $GOPATH/bin/tigrisfs <bucket> <mountpoint>
$ $GOPATH/bin/tigrisfs [--endpoint https://...] <bucket:prefix> <mountpoint> # if you only want to mount objects under a prefix
```
...
```
bucket    /mnt/mountpoint    fuse.tigrisfs    _netdev,allow_other,--file-mode=0666,--dir-mode=0777    0    0
```

You can also use a different path to the credentials file by adding `,--shared-config=/path/to/credentials`.

See also: [Instruction for Azure Blob Storage](https://github.com/yandex-cloud/tigrisfs/blob/master/README-azure.md).

## Windows

Everything is the same after installing [WinFSP](https://winfsp.dev) and TigrisFS, except that TigrisFS
...
And then start TigrisFS with `tigrisfs <bucket> <mountpoint>`, where `<mountpoint>` is either a drive
(like K:) or a non-existing directory. Example:

```
tigrisfs-win-x64.exe testbucket K:
```
...
There's a lot of tuning you can do. Consult `tigrisfs -h` to view the list of options.
...
TigrisFS uses RAM for two purposes:
...
1. **Metadata** (file listings). One metadata entry uses ~1 KB of data. Total
   ...
   default, but in reality TigrisFS is able to list files faster, so the actual
   limit will be ~250000 when doing a plain listing of a very long bucket.

2. **Data**. Default data cache limit in TigrisFS is 1 GB (`--memory-limit`).
   TigrisFS uses cache for both read buffers when it needs to load data from the
   ...
   of buffers and in that case TigrisFS will return ENOMEM errors to some of them.
...
and start tigrisfs with the following options:

```
tigrisfs --no-checksum --memory-limit 4000 \
...
TigrisFS doesn't support concurrent updates of the same file from multiple hosts. If you try to
...
This forces TigrisFS to recheck file/directory state from the server. If you don't do that
...
Normally, TigrisFS retries requests failed in this way, but if you don't want it
to retry them, you can also choose to drop the cached update by enabling
`--drop-patch-conflicts`.

## Asynchronous Write Errors

TigrisFS buffers updates in memory (or disk cache, if enabled) and flushes them asynchronously,
so writers don't get an error from an unsuccessful write. When an error occurs, TigrisFS keeps
modifications in the cache and retries to flush them to the server later. TigrisFS tries to
flush the data forever until success or until you stop TigrisFS mount process. If there is
...
Calling `fsync` on a directory makes TigrisFS flush all changes inside it. It's stricter than
...
don't work with TigrisFS because they aren't wired up in FUSE at all.

## Troubleshooting

If you experience any problems with TigrisFS - if it crashes, hangs or does something else nasty:

- Update to the latest version if you haven't already done it
- Check your system log (syslog/journalctl) and dmesg for error messages from TigrisFS
- Try to start TigrisFS in debug mode: `--debug_s3 --debug_fuse --log-file /path/to/log.txt`,
...
- If you experience crashes, you can also collect a core dump and send it to us:
  ...
  - Start tigrisfs with `GOTRACEBACK=crash` environment variable
...
Important note: you should mount tigrisfs with `--list-type 2` or `--list-type 1` options
...
* Other related fuse filesystems
  * [goofys](https://github.com/kahing/goofys): TigrisFS is a fork of Goofys.
