...
func getCgroupAvailableMem() (retVal uint64, err error) {
	...
	if err != nil {
		mainLog.Debugf("Unable to read file %s error: %s", CGROUP_PATH, err)
		return 0, err
	}
	...
	if err != nil {
		mainLog.Debugf("Unable to get memory limit from cgroup file %v error: %v", path, err)
		return 0, err
	}

	mem_usage, err := readFileAndGetValue(filepath.Join(path, MEM_USAGE_FILE_SUFFIX))
	if err != nil {
		mainLog.Debugf("Unable to get memory usage from cgroup file %v error: %v", path, err)
		return 0, err
	}
...
func readFileAndGetValue(path string) (uint64, error) {
	...
	if err != nil {
		mainLog.Debugf("Unable to read file %v error: %v", path, err)
		return 0, err
	}
